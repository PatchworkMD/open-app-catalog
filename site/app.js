...
function filtered(items) {
  ...
  const out = items.filter(x => (!q || [x.name,x.title,x.category,x.id].join(' ').toLowerCase().includes(q)) && (!cat || x.category === cat));
...
function docs() {
  return `<div class="doc"><h2>Public listings. Clear limits.</h2><p>Hugging App brings together apps and screenshots from Apple's public US App Store feeds. Screenshots are developer-published listing images, not verified recordings of a complete app experience.</p><h3>Where the data comes from</h3><p>App rankings come from category-level free-app charts. Metadata and screenshots come from Apple's Lookup API. Media is cached on this catalog's domain. An app appearing in multiple categories is kept in the first category encountered.</p><p><a href="https://rss.marketingtools.apple.com/" target="_blank" rel="noopener noreferrer">Apple chart feeds ↗</a> · <a href="https://performance-partners.apple.com/search-api" target="_blank" rel="noopener noreferrer">Apple Search API ↗</a></p><h3>What this doesn't tell you</h3><p>Listing screenshots do not prove keyboard access, real task flows, accessibility compliance, or current in-app behavior. Flows and UI elements need manual curation and are not captured by this pipeline.</p><h3>No verified revenue data</h3><p>The raw export retains legacy revenue heuristics for compatibility. These are arbitrary category baselines halved every five ranks, not measured earnings. Hugging App does not use them to compare businesses.</p><h3>Your board stays in this browser</h3><p>Saved screenshot IDs are stored on this device. Export a board to keep a copy or supply it to your agent. There is no account sync. Your agent host's policies apply to anything you share with it.</p><h3>Freshness and attribution</h3><p>The timestamp above records when the dataset was built; it is not proof that a scheduled update succeeded today. Screenshots and trademarks belong to their owners. Public availability is not a reuse licence.</p><p><a href="https://github.com/PatchworkMD/open-app-catalog" target="_blank" rel="noopener noreferrer">Catalog source ↗</a></p></div>`;
}
function pluginDocs() {
  return `<div class="doc"><h2>Review your references.</h2><p>Give your agent screenshots, interface text, source code, or an exported board. Ask for up to three prioritized improvements, each tied to evidence and a way to test it.</p><ol><li>Save useful screenshots to your board.</li><li>Export your references and attach the relevant images or code to your agent.</li><li>Ask: “Use Hugging App to review these references. Cite the evidence and mark unseen states as unverified.”</li></ol><p>The plugin reviews material you supply. It does not automatically browse this catalog or fetch third-party libraries. Your host processes supplied content under its own policies.</p><p><a class="text-link" href="https://github.com/PatchworkMD/app-design-research" target="_blank" rel="noopener noreferrer">Plugin source & installation ↗</a></p><p class="meta">Previously App Design Research. The package identifier remains <code>app-design-research</code> for existing installations. The Hugging App rename is a local release candidate until published.</p></div>`;
}
function fillHero() {
...
function render() {
  ...
  $('#content').innerHTML = items.length ? items.slice(0,limit).map(x => r === 'apps' ? appCard(x) : screenCard(x)).join('') : `<div class="empty">${empty}</div>`;
}
function view(id) {
...
document.addEventListener('click', e => {
  const b = e.target.closest('[data-save],[data-select],[data-view],[data-related],[data-reset]'); if (!b || !data) return;
...
fetch('data.json').then(r => { if (!r.ok) throw Error('Catalog unavailable'); return r.json(); }).then(d => {
  ...
  const cats = [...new Set([...data.apps,...data.screens].map(x => x.category).filter(Boolean))].sort();
